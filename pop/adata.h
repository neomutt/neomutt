...
{
  struct Connection *conn;                ///< Connection to POP server
  unsigned int status   : 2;              ///< Connection status
  bool capabilities     : 1;              ///< Server capabilities retrieved
  unsigned int use_stls : 2;              ///< Use STLS
  ...
  bool clear_cache      : 1;              ///< Clear the cache
  ...
  char *timestamp;                        ///< APOP timestamp
  struct BodyCache *bcache;               ///< body cache
  char err_msg[POP_CMD_RESPONSE];         ///< Last error message
  struct PopCache cache[POP_CACHE_LEN];   ///< Message cache
};
