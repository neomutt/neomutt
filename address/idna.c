...
char *mutt_idna_intl_to_local(const char *user, const char *domain, uint8_t flags)
{
  ...
  if (mutt_ch_convert_string(&local_user, "utf-8", cc_charset(), MUTT_ICONV_NONE) != 0)
    goto cleanup;

  if (mutt_ch_convert_string(&local_domain, "utf-8", cc_charset(), MUTT_ICONV_NONE) != 0)
    goto cleanup;
  ...
  if ((flags & MI_MAY_BE_IRREVERSIBLE) == 0)
  {
    ...
    if (mutt_ch_convert_string(&reversed_user, cc_charset(), "utf-8", MUTT_ICONV_NONE) != 0)
    {
    ...
    if (mutt_ch_convert_string(&reversed_domain, cc_charset(), "utf-8", MUTT_ICONV_NONE) != 0)
    {
...
char *mutt_idna_local_to_intl(const char *user, const char *domain)
{
  ...
  /* we don't want charset-hook effects, so we set flags to 0 */
  if (mutt_ch_convert_string(&intl_user, cc_charset(), "utf-8", MUTT_ICONV_NONE) != 0)
    goto cleanup;

  if (mutt_ch_convert_string(&intl_domain, cc_charset(), "utf-8", MUTT_ICONV_NONE) != 0)
    goto cleanup;
