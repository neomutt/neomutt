...
// email/handler.c: decodes a parsed MIME body
extern int mutt_body_handler(struct Body *b, struct State *state);
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
  ...
  // Decode/render the parsed body tree. fp_in must stay open: Body parts
  // reference byte offsets in the message file.
  if (e->body)
  {
