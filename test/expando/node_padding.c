...
void test_expando_node_padding(void)
{
  ...
  // void node_padding_repad(struct ExpandoNode **parent);
  {
    static const char *TestStrings[][2] = {
      ...
      { "%|X",                  "<PAD:EOL:'X':|>" },
      { "%a%|X",                "<PAD:EOL:'X':<EXP:'a'(ALIAS,ADDRESS)>|>" },
      { "%a%b%|X",              "<PAD:EOL:'X':<EXP:'a'(ALIAS,ADDRESS)><EXP:'b'(ALIAS,COMMENT)>|>" },
      { "%|X%c",                "<PAD:EOL:'X':|<EXP:'c'(ALIAS,FLAGS)>>" },
      { "%|X%c%d",              "<PAD:EOL:'X':|<EXP:'c'(ALIAS,FLAGS)><EXP:'d'(ALIAS,NAME)>>" },
      { "%a%|X%c",              "<PAD:EOL:'X':<EXP:'a'(ALIAS,ADDRESS)>|<EXP:'c'(ALIAS,FLAGS)>>" },
      { "%a%b%|X%c%d",          "<PAD:EOL:'X':<EXP:'a'(ALIAS,ADDRESS)><EXP:'b'(ALIAS,COMMENT)>|<EXP:'c'(ALIAS,FLAGS)><EXP:'d'(ALIAS,NAME)>>" },
      { "%<a?%a%|X%b&%c%|X%d>", "<COND:<BOOL:'a':(ALIAS,ADDRESS)>|<PAD:EOL:'X':<EXP:'a'(ALIAS,ADDRESS)>|<EXP:'b'(ALIAS,COMMENT)>>|<PAD:EOL:'X':<EXP:'c'(ALIAS,FLAGS)>|<EXP:'d'(ALIAS,NAME)>>>" },

      { "%>X",                  "<PAD:HARD:'X':|>" },
      { "%a%>X",                "<PAD:HARD:'X':<EXP:'a'(ALIAS,ADDRESS)>|>" },
      { "%a%b%>X",              "<PAD:HARD:'X':<EXP:'a'(ALIAS,ADDRESS)><EXP:'b'(ALIAS,COMMENT)>|>" },
      { "%>X%c",                "<PAD:HARD:'X':|<EXP:'c'(ALIAS,FLAGS)>>" },
      { "%>X%c%d",              "<PAD:HARD:'X':|<EXP:'c'(ALIAS,FLAGS)><EXP:'d'(ALIAS,NAME)>>" },
      { "%a%>X%c",              "<PAD:HARD:'X':<EXP:'a'(ALIAS,ADDRESS)>|<EXP:'c'(ALIAS,FLAGS)>>" },
      { "%a%b%>X%c%d",          "<PAD:HARD:'X':<EXP:'a'(ALIAS,ADDRESS)><EXP:'b'(ALIAS,COMMENT)>|<EXP:'c'(ALIAS,FLAGS)><EXP:'d'(ALIAS,NAME)>>" },
      { "%<a?%a%>X%b&%c%>X%d>", "<PAD:HARD:'X':<COND:<BOOL:'a':(ALIAS,ADDRESS)>|<PAD:HARD:'X':<EXP:'a'(ALIAS,ADDRESS)>|>|><TEXT:'X'><EXP:'b'(ALIAS,COMMENT)><TEXT:'&'><EXP:'c'(ALIAS,FLAGS)>|<EXP:'d'(ALIAS,NAME)><TEXT:'>'>>" },

      { "%*X",                  "<PAD:SOFT:'X':|>" },
      { "%a%*X",                "<PAD:SOFT:'X':<EXP:'a'(ALIAS,ADDRESS)>|>" },
      { "%a%b%*X",              "<PAD:SOFT:'X':<EXP:'a'(ALIAS,ADDRESS)><EXP:'b'(ALIAS,COMMENT)>|>" },
      { "%*X%c",                "<PAD:SOFT:'X':|<EXP:'c'(ALIAS,FLAGS)>>" },
      { "%*X%c%d",              "<PAD:SOFT:'X':|<EXP:'c'(ALIAS,FLAGS)><EXP:'d'(ALIAS,NAME)>>" },
      { "%a%*X%c",              "<PAD:SOFT:'X':<EXP:'a'(ALIAS,ADDRESS)>|<EXP:'c'(ALIAS,FLAGS)>>" },
      { "%a%b%*X%c%d",          "<PAD:SOFT:'X':<EXP:'a'(ALIAS,ADDRESS)><EXP:'b'(ALIAS,COMMENT)>|<EXP:'c'(ALIAS,FLAGS)><EXP:'d'(ALIAS,NAME)>>" },
      { "%<a?%a%*X%b&%c%*X%d>", "<COND:<BOOL:'a':(ALIAS,ADDRESS)>|<PAD:SOFT:'X':<EXP:'a'(ALIAS,ADDRESS)>|<EXP:'b'(ALIAS,COMMENT)>>|<PAD:SOFT:'X':<EXP:'c'(ALIAS,FLAGS)>|<EXP:'d'(ALIAS,NAME)>>>" },
      // clang-format off
    };
  ...
  // int node_padding_render_eol(const struct ExpandoNode *node, const struct ExpandoRenderData *rdata, struct Buffer *buf, int max_cols, void *data, MuttFormatFlags flags);
  {
    ...
    for (int i = 0; i < mutt_array_size(EolTests); i++)
    {
      ...
      TEST_CHECK(buf_is_empty(err));
      TEST_MSG(buf_string(err));
      rc = node_padding_render_eol(exp->tree, TestRenderData, buf, 19, NULL, MUTT_FORMAT_NO_FLAGS);
      TEST_CHECK(rc == 19);
      TEST_CHECK_STR_EQ(buf_string(buf), expected);
      expando_free(&exp);
    }
  ...
  // int node_padding_render_hard(const struct ExpandoNode *node, const struct ExpandoRenderData *rdata, struct Buffer *buf, int max_cols, void *data, MuttFormatFlags flags);
  {
    ...
    for (int i = 0; i < mutt_array_size(HardTests); i++)
    {
      ...
      TEST_CHECK(buf_is_empty(err));
      TEST_MSG(buf_string(err));
      rc = node_padding_render_hard(exp->tree, TestRenderData, buf, 19, NULL, MUTT_FORMAT_NO_FLAGS);
      TEST_CHECK(rc == 19);
      TEST_CHECK_STR_EQ(buf_string(buf), expected);
      expando_free(&exp);
    }
  ...
  // int node_padding_render_soft(const struct ExpandoNode *node, const struct ExpandoRenderData *rdata, struct Buffer *buf, int max_cols, void *data, MuttFormatFlags flags);
  {
    ...
    for (int i = 0; i < mutt_array_size(SoftTests); i++)
    {
      ...
      TEST_CHECK(buf_is_empty(err));
      TEST_MSG(buf_string(err));
      rc = node_padding_render_soft(exp->tree, TestRenderData, buf, 19, NULL, MUTT_FORMAT_NO_FLAGS);
      TEST_CHECK(rc == 19);
      TEST_CHECK_STR_EQ(buf_string(buf), expected);
      expando_free(&exp);
    }
