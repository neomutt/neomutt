/**
 * @file
 * Test code for mutt_mem_recalloc()
 *
 * @authors
...
void test_mutt_mem_recalloc(void)
{
...

  {
    ...
    mutt_mem_recalloc(NULL, cur_size, new_size);

    TEST_CHECK_(1, "mutt_mem_recalloc(NULL, 1024, 2048)");
  }

  {
    ...
    mutt_mem_recalloc(&ptr, cur_size, new_size);

    TEST_CHECK_(1, "mutt_mem_recalloc(&ptr, 0, 0)");
  }

  {
    ...
    TEST_CHECK_(1, "mutt_mem_recalloc(&ptr, 0, 1024)");
    TEST_CHECK(ptr != NULL);
...

  {
    ...
    TEST_CHECK_(1, "mutt_mem_recalloc(&ptr, 1024, 1024)");
    TEST_CHECK(ptr != NULL);
...

  {
    ...
    mutt_mem_recalloc(&ptr, cur_size, new_size);
    TEST_CHECK_(1, "mutt_mem_recalloc(&ptr, 1024, 2048)");
    TEST_CHECK(ptr != NULL);
...

  {
    ...
    mutt_mem_recalloc(&ptr, cur_size, new_size);
    TEST_CHECK_(1, "mutt_mem_recalloc(&ptr, 2048, 1024)");
    TEST_CHECK(ptr != NULL);
...

  {
    ...
    mutt_mem_recalloc(&ptr, cur_size, new_size);
    TEST_CHECK_(1, "mutt_mem_recalloc(&ptr, 1024, 0)");
    TEST_CHECK(ptr == NULL);
