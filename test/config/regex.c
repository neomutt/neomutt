...
static bool test_initial_values(struct ConfigSet *cs, struct Buffer *err)
{
  ...
  if (!TEST_CHECK(mutt_str_equal(VarApple->pattern, "apple.*", CASE_MATCH)))
  {
    TEST_MSG("Error: initial values were wrong\n");
    return false;
  }

  if (!TEST_CHECK(mutt_str_equal(VarBanana->pattern, "banana.*", CASE_MATCH)))
  {
    TEST_MSG("Error: initial values were wrong\n");
    return false;
  }
...
static bool test_string_set(struct ConfigSet *cs, struct Buffer *err)
{
  ...
  for (unsigned int i = 0; i < mutt_array_size(valid); i++)
  {
    ...
    if (!TEST_CHECK(mutt_str_equal(regex, valid[i], CASE_MATCH)))
    {
      TEST_MSG("Value of %s wasn't changed\n", name);
      return false;
    }
...
static bool test_native_set(struct ConfigSet *cs, struct Buffer *err)
{
  ...
  if (!TEST_CHECK(mutt_str_equal(regex, r->pattern, CASE_MATCH)))
  {
    TEST_MSG("Value of %s wasn't changed\n", name);
    goto tns_out;
  }
...
static bool test_reset(struct ConfigSet *cs, struct Buffer *err)
{
  ...
  regex = VarMango ? VarMango->pattern : NULL;
  if (!TEST_CHECK(mutt_str_equal(regex, "mango.*", CASE_MATCH)))
  {
    TEST_MSG("Value of %s wasn't changed\n", name);
    return false;
  }
  ...
  if (!TEST_CHECK(mutt_str_equal(VarOlive->pattern, "hel*o", CASE_MATCH)))
  {
