...
static bool test_initial_values(struct ConfigSet *cs, struct Buffer *err)
{
  ...
  {
    if (!mutt_str_equal(values[i], np->data, CASE_MATCH))
      return false;
    i++;
  }
...
static bool test_string_get(struct ConfigSet *cs, struct Buffer *err)
{
  ...
  if (!mutt_str_equal(initial.data, err->data, CASE_MATCH))
  {
...
static bool test_reset(struct ConfigSet *cs, struct Buffer *err)
{
  ...
  item = STAILQ_FIRST(&VarLemon->head)->data;
  if (!TEST_CHECK(mutt_str_equal(item, "lemon", CASE_MATCH)))
  {
  ...
  item = STAILQ_FIRST(&VarMango->head)->data;
  if (!TEST_CHECK(mutt_str_equal(item, "banana", CASE_MATCH)))
  {
