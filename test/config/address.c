...
static bool test_initial_values(struct ConfigSet *cs, struct Buffer *err)
{
  ...
  if (!TEST_CHECK(mutt_str_equal(VarApple->mailbox, apple_orig, CASE_MATCH)))
  {
    TEST_MSG("Error: initial values were wrong\n");
    return false;
  }

  if (!TEST_CHECK(mutt_str_equal(VarBanana->mailbox, banana_orig, CASE_MATCH)))
  {
    TEST_MSG("Error: initial values were wrong\n");
    return false;
  }
  ...
  if (!TEST_CHECK(mutt_str_equal(value.data, apple_orig, CASE_MATCH)))
  {
  ...
  if (!TEST_CHECK(mutt_str_equal(value.data, banana_orig, CASE_MATCH)))
  {
...
static bool test_string_set(struct ConfigSet *cs, struct Buffer *err)
{
  ...
  for (unsigned int i = 0; i < mutt_array_size(valid); i++)
  {
    ...
    if (!TEST_CHECK(mutt_str_equal(addr, valid[i], CASE_MATCH)))
    {
...
static bool test_native_set(struct ConfigSet *cs, struct Buffer *err)
{
  ...
  if (!TEST_CHECK(mutt_str_equal(addr, a->mailbox, CASE_MATCH)))
  {
    TEST_MSG("Value of %s wasn't changed\n", name);
    goto tbns_out;
  }
...
static bool test_reset(struct ConfigSet *cs, struct Buffer *err)
{
  ...
  addr = VarLemon ? VarLemon->mailbox : NULL;
  if (!TEST_CHECK(mutt_str_equal(addr, "lemon@example.com", CASE_MATCH)))
  {
    TEST_MSG("Value of %s wasn't changed\n", name);
    return false;
  }
  ...
  if (!TEST_CHECK(mutt_str_equal(VarMango->mailbox, "john@example.com", CASE_MATCH)))
  {
