...
static bool test_initial_values(struct ConfigSet *cs, struct Buffer *err)
{
  ...
  if (!TEST_CHECK(mutt_str_equal(VarApple->orig_str, "apple", CASE_MATCH)))
  {
    TEST_MSG("Error: initial values were wrong\n");
    return false;
  }

  if (!TEST_CHECK(mutt_str_equal(VarBanana->orig_str, "banana", CASE_MATCH)))
  {
    TEST_MSG("Error: initial values were wrong\n");
    return false;
  }
...
static bool test_string_set(struct ConfigSet *cs, struct Buffer *err)
{
  ...
  for (unsigned int i = 0; i < mutt_array_size(valid); i++)
  {
    ...
    if (!TEST_CHECK(mutt_str_equal(mb, valid[i], CASE_MATCH)))
    {
      TEST_MSG("Value of %s wasn't changed\n", name);
      return false;
    }
  ...
  for (unsigned int i = 0; i < mutt_array_size(valid); i++)
  {
    ...
    if (!TEST_CHECK(mutt_str_equal(orig_str, valid[i], CASE_MATCH)))
    {
      TEST_MSG("Value of %s wasn't changed\n", name);
      return false;
    }
...
static bool test_native_set(struct ConfigSet *cs, struct Buffer *err)
{
  ...
  if (!TEST_CHECK(mutt_str_equal(mb, t->orig_str, CASE_MATCH)))
  {
    TEST_MSG("Value of %s wasn't changed\n", name);
    goto tns_out;
  }
...
static bool test_reset(struct ConfigSet *cs, struct Buffer *err)
{
  ...
  mb = VarLemon ? VarLemon->orig_str : NULL;
  if (!TEST_CHECK(mutt_str_equal(mb, "lemon", CASE_MATCH)))
  {
    TEST_MSG("Value of %s wasn't changed\n", name);
    return false;
  }
  ...
  if (!TEST_CHECK(mutt_str_equal(VarMango->orig_str, "hello", CASE_MATCH)))
  {
