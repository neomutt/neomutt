...
void test_editor_kill_word(void)
{
...

  {
    ...
    TEST_CHECK_NUM_EQ(editor_buffer_get_lastchar(es), 5);
    TEST_CHECK_NUM_EQ(editor_buffer_get_cursor(es), 5);
    enter_state_free(&es);
  }

  {
    ...
    TEST_CHECK_NUM_EQ(editor_buffer_get_cursor(es), 13);
    TEST_CHECK(editor_kill_word(es) == FR_SUCCESS);
    TEST_CHECK_NUM_EQ(editor_buffer_get_lastchar(es), 12);
    TEST_CHECK_NUM_EQ(editor_buffer_get_cursor(es), 12);
    enter_state_free(&es);
  }
...
#if 0
  {
    ...
    TEST_CHECK_NUM_EQ(editor_buffer_get_cursor(es), 10);
    TEST_CHECK(editor_kill_word(es) == FR_SUCCESS);
    TEST_CHECK_NUM_EQ(editor_buffer_get_lastchar(es), 12);
...

  {
    ...
    TEST_CHECK(editor_kill_word(es) == FR_SUCCESS);
    TEST_CHECK_NUM_EQ(editor_buffer_get_lastchar(es), 8);
