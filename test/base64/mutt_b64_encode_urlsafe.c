/**
 * @file
 * Test code for mutt_b64_encode_urlsafe()
 *
 * @authors
...
void test_mutt_b64_encode_urlsafe(void)
{
  // size_t mutt_b64_encode_urlsafe(const char *in, size_t inlen, char *out, size_t outlen);

  {
    char buf[32] = { 0 };
    TEST_CHECK(mutt_b64_encode_urlsafe(NULL, 5, buf, sizeof(buf)) == 0);
  }

  {
    TEST_CHECK(mutt_b64_encode_urlsafe("apple", 5, NULL, 10) == 0);
  }

  {
    char buffer[16] = { 0 };
    size_t len = mutt_b64_encode_urlsafe(clear, sizeof(clear) - 1, buffer, sizeof(buffer));
...

  {
    ...
    enclen = mutt_b64_encode_urlsafe(in, 0, out1, 32);
...

  {
    ...
    {
      size_t enclen = mutt_b64_encode_urlsafe(in, i, out1, 32);
      size_t exp = ((i + 2) / 3) << 2;
