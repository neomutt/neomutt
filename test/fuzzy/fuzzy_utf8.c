...
void test_fuzzy_utf8_bytewise_matching(void)
{
  ...
  // UTF-8 codepoints must match as complete contiguous sequences
  score = fuzzy_match("é", "Ã©", FUZZY_ALGO_SUBSEQ, &opts, &result);
  TEST_CHECK(score < 0);
  TEST_MSG("UTF-8 codepoint bytes must not match across different characters");
