/**
 * @file
 * Test code for parse_unsubscribe_from()
 *
 * @authors
...
static const struct Command UnSubscribeFrom = { "unsubscribe-from",
                                                CMD_UNSUBSCRIBE_FROM, NULL };
...
void test_parse_unsubscribe_from(void)
{
  ...
  for (int i = 0; Tests[i].line; i++)
  {
    ...
    buf_seek(line, 0);
    rc = parse_unsubscribe_from(&UnSubscribeFrom, line, pc, pe);
