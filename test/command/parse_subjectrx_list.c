/**
 * @file
 * Test code for parse_subjectrx_list()
 *
 * @authors
...
// clang-format off
static const struct Command SubjectRegex = { "subject-regex", CMD_SUBJECT_REGEX, NULL};
// clang-format on
...
void test_parse_subjectrx_list(void)
{
  ...
  for (int i = 0; Tests[i].line; i++)
  {
    ...
    buf_seek(line, 0);
    rc = parse_subjectrx_list(&SubjectRegex, line, pc, pe);
