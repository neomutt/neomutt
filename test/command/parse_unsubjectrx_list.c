/**
 * @file
 * Test code for parse_unsubjectrx_list()
 *
 * @authors
...
// clang-format off
static const struct Command UnSubjectRegex = { "unsubject-regex", CMD_UNSUBJECT_REGEX, NULL};
// clang-format on
...
void test_parse_unsubjectrx_list(void)
{
  ...
  for (int i = 0; Tests[i].line; i++)
  {
    ...
    buf_seek(line, 0);
    rc = parse_unsubjectrx_list(&UnSubjectRegex, line, pc, pe);
