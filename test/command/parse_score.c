...
static const struct Command Score = { "score", CMD_SCORE, NULL };

static const struct CommandTest Tests[] = {
  // clang-format off
  // score <pattern> <value>
  { MUTT_CMD_WARNING, "" },
  { MUTT_CMD_SUCCESS, "'~s patch' +70" },
  { MUTT_CMD_SUCCESS, "'~f rr' 19" },
  { MUTT_CMD_ERROR,   NULL },
  // clang-format on
};

void test_parse_score(void)
{
  ...
  for (int i = 0; Tests[i].line; i++)
  {
    ...
    buf_seek(line, 0);
    rc = parse_score(&Score, line, pc, pe);
