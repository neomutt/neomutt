...
static const struct Command Finish = { "finish", CMD_FINISH, NULL, CMD_NO_DATA };
...
void test_parse_finish(void)
{
  ...
  for (int i = 0; Tests[i].line; i++)
  {
    ...
    buf_seek(line, 0);
    rc = parse_finish(&Finish, line, NULL, NULL);
