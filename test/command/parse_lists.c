...
static const struct Command Lists = { "lists", CMD_LISTS, NULL, CMD_NO_DATA };
...
static const struct CommandTest Tests[] = {
  ...
  { MUTT_CMD_SUCCESS, "-group work neomutt-.*@neomutt.org" },
  { MUTT_CMD_ERROR,   NULL },
  // clang-format on
};

void test_parse_lists(void)
{
  ...
  for (int i = 0; Tests[i].line; i++)
  {
    ...
    buf_seek(line, 0);
    rc = parse_lists(&Lists, line, NULL, NULL);
