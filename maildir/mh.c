 ...
 * @retval -2 Aborted
 */
int mh_parse_dir(struct Mailbox *m, struct MdEmail ***last, int *count, struct Progress *progress)
{
  struct dirent *de = NULL;
  int rc = 0;
  bool is_old = false;
 ...
 * maildir parsing.
 */
struct Email *mh_parse_message(enum MailboxType type, const char *fname, struct Email *e)
{
...
#endif
    {
      if (mh_parse_message(m->type, fn, p->email))
      {
...
int mh_read_dir(struct Mailbox *m)
{
  ...
  last = &md;
  int count = 0;
  if (mh_parse_dir(m, &last, &count, &progress) < 0)
    return -1;
...
int mh_mbox_check(struct Mailbox *m)
{
  ...
  mh_parse_dir(m, &last, &count, NULL);
