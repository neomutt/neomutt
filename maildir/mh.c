 ...
 * @retval -2 Aborted
 */
int mh_parse_dir(struct Mailbox *m, struct MdEmail ***last, const char *subdir,
                 int *count, struct Progress *progress)
{
  struct dirent *de = NULL;
  int rc = 0;
  bool is_old = false;
 ...
 * maildir parsing.
 */
struct Email *mh_parse_message(enum MailboxType type, const char *fname,
...
#endif
    {
      if (mh_parse_message(m->type, fn, p->email->old, p->email))
      {
...
int mh_read_dir(struct Mailbox *m, const char *subdir)
{
  ...
  last = &md;
  int count = 0;
  if (mh_parse_dir(m, &last, subdir, &count, &progress) < 0)
    return -1;
  ...
  mh_delayed_parsing(m, &md, &progress);
 ...
 * @retval -1 Failure
 */
int mh_open_message(struct Mailbox *m, struct Message *msg, int msgno, bool is_maildir)
{
  if (!m || !m->emails || (msgno >= m->msg_count))
    return -1;
...
int mh_mbox_check(struct Mailbox *m)
{
  ...
  mh_parse_dir(m, &last, NULL, &count, NULL);
  mh_delayed_parsing(m, &md, NULL);
...
{
  return mh_open_message(m, msg, msgno, false);
}
