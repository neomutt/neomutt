...
function launch_unit_tests_runner () {
  ...
  for spec_source in ${SOURCE_DIR}/tests/unit/*_spec.c; do
    local spec_runtime=${spec_source/.c/}
    gcc --std=c99 -o ${spec_runtime} ${spec_source} -fprofile-arcs -ftest-coverage \
        -I${SOURCE_DIR}/tests/unit/ext -I${SOURCE_DIR} -I${BUILD_DIR}    \
...
echo
check_ret "All tests went fine" "Failure running unit tests"
