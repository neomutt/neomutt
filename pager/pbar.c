...
static int pbar_recalc(struct MuttWindow *win)
{
  ...
  {
    int msg_in_pager = shared->ctx->msg_in_pager;
...
static int pbar_repaint(struct MuttWindow *win)
{
  ...
  mutt_window_move(win, 0, 0);
  mutt_draw_statusline(win->state.cols, pbar_data->pager_format,
...
/**
 * pbar_add - Add the Pager Bar
 * @param parent Parent Window
 * @param shared Shared Pager data
 * @param priv   Private Pager data
 */
struct MuttWindow *pbar_add(struct MuttWindow *parent, struct IndexSharedData *shared,
                            struct PagerPrivateData *priv)
{
  ...
  mutt_window_add_child(parent, win_pbar);
