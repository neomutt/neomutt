                       ...
                       struct Email **cur, struct Buffer *fcc)
{
  ...
  {
    size_t plen = 0;
    /* Check for header names from least unambiguous to most ambiguous.
     * Note: If a string comparison succeeds, `plen` is not overridden by
     * the following ones due to the short-circuit behaviour of `||`. */
