...
static int lua_global_cb_enter(lua_State *l)
{
  ...
  enum CommandResult rc = parse_rc_line(line, pc, pe);
  buf_pool_release(&line);

  char text[1024] = { 0 };
  mutt_str_copy(text, buf_string(pe->message), sizeof(text));
  parse_error_free(&pe);
  parse_context_free(&pc);

  if (rc == MUTT_CMD_SUCCESS)
  {
    if (!lua_pushstring(l, text))
      lua_handle_error(l);
    else
      rc++;
  }
  ...
  {
    // mutt_error("AAA NeoMutt error: %s", text);
    // luaL_error(l, "AAA NeoMutt error: %s", text);
    luaL_error(l, "%s", text);
    // or lua_pushstring(l, text); lua_error(l);
    rc = -1;
  }
