...
int lua_console_make_entry(struct Menu *menu, int line, int max_cols, struct Buffer *buf)
{
  ...
  struct LuaLogFile *llf = mod_data->log_file;
  if (!llf)
    return 0;
...
struct MuttWindow *lua_console_init(void)
{
  ...
  struct MuttWindow *win = menu_window_new(menu_find(MENU_LUA), NeoMutt->sub);
...
void lua_console_update(void)
{
  ...
  struct LuaConsoleInfo *lci = mod_data->console;
  if (!lci || !lci->menu)
    return;
