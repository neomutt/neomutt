                        ...
                        void (*callback)(void *, const char *), void *data)
{
  ...
  struct EnterWindowData wdata = {
    buf, 0, es, 0, NULL, NULL, prompt, ENTER_REDRAW_NONE, MUTT_COMP_NO_FLAGS, true, NULL, 0, &mbstate, 0, false, NULL, 0, 0
  };
  ...
  do
  {
    ...
    do
    {
      ...
      window_redraw(NULL);
      struct KeyEvent event = km_dokey(MENU_SIDEBAR, GETCH_IGNORE_MACRO);
      if ((event.op == OP_TIMEOUT) || (event.op == OP_REPAINT))
      {
        continue;
      }
      ...
      int rc_disp = sb_function_dispatcher(data, &event);
