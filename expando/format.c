...
void buf_justify(struct Buffer *buf, enum FormatJustify justify, size_t cols, char pad_char)
{
  ...
  switch (justify)
  {
    ...
    case JUSTIFY_CENTER:
    {
      if (cols > 1)
      {
        memmove(buf->data + (cols / 2), buf->data, len);
        memset(buf->data, pad_char, cols / 2);
      }
      memset(buf->data + len + (cols / 2), pad_char, (cols + 1) / 2);
      break;
    }

    case JUSTIFY_RIGHT:
    {
      memmove(buf->data + cols, buf->data, len);
      memset(buf->data, pad_char, cols);
      break;
    }
  }
 ...
 * except that the cols refer to the number of character cells when printed.
 */
