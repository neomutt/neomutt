 ...
 *
 * @authors
 * Copyright (C) 2017 Anton Rieger <seishinryohosha@jikken.de>
 * Copyright (C) 2017-2023 Richard Russon <rich@flatcap.org>
 * Copyright (C) 2018 Reis Radomil
 * Copyright (C) 2019 David Harrigan <dharrigan@gmail.com>
 ...
 * @param menu Menu listing Attachments
 * @param op   Operation, e.g. OP_ATT_DISPLAY_ATTACHMENT_DEFAULT
 * @param e  Email
 * @param actx Attachment context
                             ...
                             struct Email *e, struct AttachCtx *actx, bool recv)
{
  do
  {
    switch (op)
    {
      case OP_ATT_DISPLAY_MESSAGE_HEADERS:
        bool_str_toggle(NeoMutt->sub, "weed", NULL);
        FALLTHROUGH;

      case OP_ATT_DISPLAY_ATTACHMENT_DEFAULT:
      {
        struct AttachPtr *cur_att = current_attachment(actx, menu);
        if (!cur_att->fp)
        {
      ...
      case OP_SELECT_NEXT_UNDELETED_ENTRY: /* hack */
      {
        ...
        if (index < menu->max)
        {
          ...
          op = OP_ATT_DISPLAY_ATTACHMENT_DEFAULT;
        }
        else
        {
          op = OP_NULL;
      ...
      case OP_ATT_ATTACH_EDIT_CONTENT_TYPE:
      {
        struct AttachPtr *cur_att = current_attachment(actx, menu);
      ...
      case OP_ATT_ATTACH_SAVE_ATTACHMENT:
      {
        struct AttachPtr *cur_att = current_attachment(actx, menu);
        struct AttachPtrArray aa = ARRAY_HEAD_INITIALIZER;
        ARRAY_ADD(&aa, cur_att);
