...
static const struct Mapping AttachmentHelp[] = {
  // clang-format off
  { N_("Exit"),  OP_EXIT },
  { N_("Save"),  OP_ATT_ATTACH_SAVE_ATTACHMENT },
                ...
                struct Email *e, FILE *fp, bool attach_msg)
{
  ...
  struct SimpleDialogWindows sdw = simple_dialog_new(mod_data->menu_attach,
                                                     WT_DLG_ATTACH, AttachmentHelp);
  ...
  do
  {
    ...
    if (op == OP_NULL)
    {
      km_error_key(mod_data->menu_attach);
      continue;
    }
    mutt_clear_error();
