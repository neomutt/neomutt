...
void ansi_colors_dump(struct PagedFile *pf)
{
  ...
  paged_line_add_colored_text(pl, MT_COLOR_COMMENT, _("# Ansi Colors\n"));
  ...
  ARRAY_FOREACH(ct, &cta)
  {
    ...
    len = paged_line_add_colored_text(pl, MT_COLOR_NUMBER, buf_string(buf));
...
void curses_colors_dump(struct PagedFile *pf, struct AttrColorList *acl)
{
  ...
  pl = paged_file_new_line(pf);
  paged_line_add_colored_text(pl, MT_COLOR_COMMENT, _("# Curses Colors\n"));

  pl = paged_file_new_line(pf);
  paged_line_add_colored_text(pl, MT_COLOR_COMMENT, "# Index rc fg      bg      Color\n");
  ...
  TAILQ_FOREACH(cc, &CursesColors, entries)
  {
    ...
    else
    {
      ...
      paged_line_add_colored_text(pl, MT_COLOR_COLOR, buf_string(buf));
      paged_line_add_colored_text(pl, MT_COLOR_COMMENT, " ");
    }
...
void merged_colors_dump(struct PagedFile *pf)
{
  ...
  pl = paged_file_new_line(pf);
  paged_line_add_colored_text(pl, MT_COLOR_COMMENT, _("# Merged Colors\n"));
  ...
  ARRAY_FOREACH(ct, &cta)
  {
    ...
    if (!slist_is_empty(ct->attrs))
    {
      ...
      STAILQ_FOREACH(np, &ct->attrs->head, entries)
      {
        len += paged_line_add_colored_text(pl, MT_COLOR_ATTRIBUTE, np->data);
        if (STAILQ_NEXT(np, entries))
          len += paged_line_add_colored_text(pl, MT_COLOR_COMMENT, " ");
      }
    }
    buf_printf(buf, "%*s", w_attrs - len + 1, "");
    paged_line_add_colored_text(pl, MT_COLOR_COMMENT, buf_string(buf));
