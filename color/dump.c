...
void regex_colors_dump(struct PagedFile *pf, bool all)
{
  ...
  for (enum ColorId cid = MT_COLOR_NONE; cid != MT_COLOR_MAX; cid++)
  {
    ...
    {
      pl = paged_file_new_line(pf);
      paged_line_add_colored_text(pl, MT_COLOR_FUNCTION, "color");
      paged_line_add_text(pl, " ");

      len = paged_line_add_colored_text(pl, MT_COLOR_IDENTIFIER, name);
      ...
      if (!slist_is_empty(ct->attrs))
      {
        ...
        {
          len += paged_line_add_colored_text(pl, MT_COLOR_ATTRIBUTE, np->data);
          if (STAILQ_NEXT(np, entries))
            len += paged_line_add_text(pl, " ");
        }
      }
      buf_printf(buf, "%*s", w_attrs - len + 1, "");
      paged_line_add_text(pl, buf_string(buf));

      len = paged_line_add_colored_text(pl, MT_COLOR_COLOR, ct->fg);
      buf_printf(buf, "%*s", w_fg - len + 1, "");
      paged_line_add_text(pl, buf_string(buf));

      len = paged_line_add_colored_text(pl, MT_COLOR_COLOR, ct->bg);
      buf_printf(buf, "%*s", w_bg - len + 1, "");
      paged_line_add_text(pl, buf_string(buf));

      len = paged_line_add_colored_text(pl, MT_COLOR_STRING, ct->pattern);
      ...
      if (ct->match > 0)
      {
        ...
        paged_line_add_colored_text(pl, MT_COLOR_NUMBER, buf_string(buf));
        paged_line_add_text(pl, " ");
      }
      else if (w_match > 0)
      {
      ...
      paged_line_add_raw_text(pl, buf_string(swatch));

      paged_line_add_ac_text(pl, ct->attr_color, "XXXXXX");

      paged_line_add_raw_text(pl, "\033[0m");
...
void simple_colors_dump(struct PagedFile *pf, bool all)
{
  ...
  {
    pl = paged_file_new_line(pf);
    paged_line_add_colored_text(pl, MT_COLOR_COMMENT, _("# Simple Colors\n"));
    ...
    ARRAY_FOREACH(ct, &cta)
    {
      ...
      len = paged_line_add_colored_text(pl, MT_COLOR_IDENTIFIER, ct->name);
      buf_printf(buf, "%*s", w_name - len + 1, "");
      paged_line_add_text(pl, buf_string(buf));
...
void status_colors_dump(struct PagedFile *pf, bool all)
{
  ...
  pl = paged_file_new_line(pf);
  paged_line_add_colored_text(pl, MT_COLOR_COMMENT, _("# Status Colors\n"));
  ...
  len = paged_line_add_colored_text(pl, MT_COLOR_IDENTIFIER, simple_name);
  buf_printf(buf, "%*s", w_name - len + 1, "");
  paged_line_add_text(pl, buf_string(buf));
  ...
  len = paged_line_add_colored_text(pl, MT_COLOR_COLOR, simple_fg);
  buf_printf(buf, "%*s", w_fg - len + 1, "");
  paged_line_add_text(pl, buf_string(buf));

  len = paged_line_add_colored_text(pl, MT_COLOR_COLOR, simple_bg);
  buf_printf(buf, "%*s", w_bg - len + 1, "");
  paged_line_add_text(pl, buf_string(buf));
  ...
  paged_line_add_ac_text(pl, ac, "XXXXXX");
