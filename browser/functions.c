...
static const struct MenuFuncOp OpBrowser[] = { /* map: browser */
  ...
  { "goto-directory",                OP_CHANGE_DIRECTORY },
...
static int op_browser_new_file(struct BrowserPrivateData *priv, const struct KeyEvent *event)
{
  ...
  const int rc = mw_get_field(_("New file name: "), buf, MUTT_COMP_NONE,
...
static int op_change_directory(struct BrowserPrivateData *priv, const struct KeyEvent *event)
{
  ...
  {
    struct FileCompletionData cdata = { false, priv->mailbox, NULL, NULL, NULL };
    int rc = mw_get_field(_("Chdir to: "), buf, MUTT_COMP_NONE, HC_FILE,
...
static int op_subscribe_pattern(struct BrowserPrivateData *priv, const struct KeyEvent *event)
{
  ...
  if ((mw_get_field(tmp2, buf, MUTT_COMP_NONE, HC_PATTERN, &CompletePatternOps, NULL) != 0) ||
      buf_is_empty(buf))
  {
    buf_pool_release(&buf);
    return FR_NO_ACTION;
...
static const struct BrowserFunction BrowserFunctions[] = {
  ...
  { OP_NNTP_MARK_NEWSGROUP_UNREAD,   op_catchup },
