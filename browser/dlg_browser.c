...
static int folder_make_entry(struct Menu *menu, int line, int max_cols, struct Buffer *buf)
{
  ...
  if (OptNews)
  {
    ...
    return expando_filter(c_group_index_format, GroupIndexRenderData, max_cols, buf);
  }
  ...
  if (bstate->is_mailbox_list)
  {
    ...
    return expando_filter(c_mailbox_folder_format, FolderRenderData, max_cols, buf);
  }

  const struct Expando *c_folder_format = cs_subset_expando(NeoMutt->sub, "folder_format");
  return expando_filter(c_folder_format, FolderRenderData, max_cols, buf);
}
