...
void command_set_expand_value(int type, struct Buffer *value)
{
  ...
  else if (IS_COMMAND(type))
  {
    ...
    {
      buf_expand_path_regex(scratch, false);
    }
    buf_reset(value);
...
enum CommandResult parse_set(const struct Command *cmd, struct Buffer *line, struct Buffer *err)
{
  ...
  do
  {
    bool prefix = false;
    bool query = false;
    bool inv = (cmd->id == CMD_TOGGLE);
    bool reset = (cmd->id == CMD_RESET);
    bool unset = (cmd->id == CMD_UNSET);
    ...
    else if ((*line->dptr == '+') || (*line->dptr == '-'))
    {
      ...
      if (reset || unset || inv)
      {
        buf_printf(err, _("Can't set option with the '%s' command"), cmd->name);
        goto done;
      }
      if (*line->dptr == '+')
        increment = true;
