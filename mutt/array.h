...
/**
 * ARRAY_INSERT - Insert an element into the array at the given index
 * @param head Pointer to a struct defined using ARRAY_HEAD()
 * @param idx  Index where the element should be inserted (0 = beginning, ARRAY_SIZE = end)
 * @param elem Element to insert
 * @retval true  Element was inserted
 * @retval false Index was out of bounds (idx > ARRAY_SIZE)
 *
 * @note Shifts all elements from idx onwards to the right.
 * @note idx must be between 0 and ARRAY_SIZE(head) (inclusive).
 * @note To append to the end, use ARRAY_ADD() instead.
 * @note Memory allocation failure causes program termination (via mutt_mem_reallocarray).
 */
