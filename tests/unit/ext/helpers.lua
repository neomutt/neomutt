...
local formatc = require('tests.unit.ext.formatc')
local Set = require('tests.unit.ext.set')
local Preprocess = require('tests.unit.ext.preprocess')
...
-- load mutt shared library
local libmutt = ffi.load(Paths.test_libnvim_path)
-- local test_helpers = ffi.load(Paths.test_helpers_path)

local function trim(s)
  return s:match('^%s*(.*%S)') or ''
end

-- a Set that keeps around the lines we have already seen
if cdefs == nil then
  cdefs = Set:new()
end
...
-- this helper will return the C namespace of the mutt library.
local function cimport(...)
  local paths = {}
  local args = {...}

  -- filter out paths weve already imported
  ...
  for line in body:gmatch("[^\r\n]+") do
    line = trim(line)
    -- give each #pragma pack an unique id, so that they dont get removed
  ...
  -- subtract the lines weve already imported from the new lines, then add
  -- the new unique lines to the old lines (so they wont be imported again)
  new_cdefs:diff(cdefs)
  cdefs:union(new_cdefs)

  if new_cdefs:size() == 0 then
    -- if theres no new lines, just return
    return libmutt
  end
...
cimport('config.h', 'mutt.h', 'test_helpers.h')
