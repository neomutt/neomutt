...
/**
 * utf8_is_continuation - Check for UTF-8 continuation byte
 * @param c Byte to check
 * @retval true  Byte is in range 0x80-0xBF
 * @retval false Byte is not a continuation byte
 */
...
/**
 * utf8_char_len - Get length of a UTF-8 codepoint at a byte offset
 * @param s String position
 * @retval num Number of bytes in this codepoint (1-4)
 *
...
static int utf8_char_len(const char *s)
{
  ...
  if ((c0 >= 0xC2) && (c0 <= 0xDF))
  {
    if (s[1] && utf8_is_continuation((unsigned char) s[1]))
      return 2;
    return 1;
  }
  ...
  if ((c0 >= 0xE0) && (c0 <= 0xEF))
  {
    if (s[1] && s[2] && utf8_is_continuation((unsigned char) s[1]) &&
  ...
  if ((c0 >= 0xF0) && (c0 <= 0xF4))
  {
    if (s[1] && s[2] && s[3] && utf8_is_continuation((unsigned char) s[1]) &&
        utf8_is_continuation((unsigned char) s[2]) &&
                       ...
                       const struct FuzzyOptions *opts, struct FuzzyResult *out)
{
  ...
  {
    const unsigned char pbyte = (unsigned char) pattern[pi];
    if (pbyte < 0x80)
    {
    ...
    while (candidate[ci])
    {
      const unsigned char cbyte = (unsigned char) candidate[ci];
      if (utf8_is_continuation(cbyte))
      {
        ci++;
        continue;
      }

      const int cchar_len = utf8_char_len(candidate + ci);
