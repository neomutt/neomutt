...
static const struct Mapping PgpHelp[] = {
  ...
  { N_("Check key"), OP_PGP_DISPLAY_DETAILS },
...
struct PgpKeyInfo *dlg_pgp(struct PgpKeyInfo *keys, struct Address *p, const char *s)
{
  ...
  struct SimpleDialogWindows sdw = simple_dialog_new(mod_data->menu_pgp, WT_DLG_PGP, PgpHelp);
  menu = sdw.menu;
  ...
  do
  {
    ...
    event = km_dokey(mod_data->menu_pgp, GETCH_NONE);
    op = event.op;
    ...
    if (op == OP_NULL)
    {
      km_error_key(mod_data->menu_pgp);
      continue;
    }
    mutt_clear_error();
