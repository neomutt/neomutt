...
static int spager_set_top_row(struct SimplePagerWindowData *wdata, int vrow)
{
  ...
  const int vcount = paged_rows_count_virtual_rows(&pf->rows);
...
static int op_spager_bottom(struct MuttWindow *win, int op)
{
  ...
  // Pick a new top, so the last entry is on the bottom row
  const int vcount = paged_rows_count_virtual_rows(&pf->rows);
...
/**
 * op_spager_next_line - Scroll down one row - Implements ::spager_function_t - @ingroup spager_function_api
 */
...
/**
 * op_spager_prev_line - Scroll up one row - Implements ::spager_function_t - @ingroup spager_function_api
 */
...
static int op_spager_search(struct MuttWindow *win, int op)
{
  ...
  spager_search_set_rows(search, &wdata->paged_file->rows);
