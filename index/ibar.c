...
static int ibar_repaint(struct MuttWindow *win)
{
  ...
  mutt_window_move(win, 0, 0);
  mutt_draw_statusline(win->state.cols, ibar_data->status_format,
...
/**
 * ibar_add - Add the Index Bar (status)
 * @param parent Parent Window
 * @param shared Shared Index data
 * @param priv   Private Index data
 */
void ibar_add(struct MuttWindow *parent, struct IndexSharedData *shared,
              struct IndexPrivateData *priv)
{
  ...
  mutt_window_add_child(parent, win_ibar);
