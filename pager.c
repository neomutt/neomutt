                       ...
                       int *pcol, int *pspecial, struct MuttWindow *pager_window)
{
  ...
  mutt_debug(2, "pager: fmt wrap? %d Wrap %d wc %d\n",
...
static void pager_menu_redraw(struct Menu *pager_menu)
{
  ...
  if ((pager_menu->redraw & REDRAW_BODY) || rd->topline != rd->oldtopline)
  {
    do
    {
...
#else
      if (COLS > col_min_width)
        col_max = COLS / col_min_width;
#endif
      ...
      while (lines2 < bodylen2 && rd->line_info[rd->curline].offset <= rd->sb.st_size - 1)
      {
        if (col > col_max)
          break;
        ...
        {
#ifdef USE_SIDEBAR
          mutt_debug(2, "pager: cols %d sw %d cshift %d wrap %d\n", COLS,
                     SidebarWidth, col_shift, Wrap);
#else
          mutt_debug(2, "pager: cols %d sw %d cshift %d wrap %d\n", COLS, -1,
                     col_shift, Wrap);
#endif
        ...
        rd->curline++;
        mutt_window_move(rd->pager_window, rd->lines + header_skip, col_woff);
      }
...
int mutt_pager(const char *banner, const char *fname, int flags, struct Pager *extra)
{
  ...
  rd.max_line = col_max * LINES; /* number of lines on screen, from curses */
