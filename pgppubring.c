...
static void pgpring_dump_signatures(struct PgpSignatureList *sig)
{
  struct PgpSignatureNode *np = NULL;
  STAILQ_FOREACH(np, sig, entriens)
  {
    if (np->sigtype == 0x10 || np->sigtype == 0x11 || np->sigtype == 0x12 ||
        np->sigtype == 0x13)
    {
      printf("sig::::%08lX%08lX::::::%X:\n", np->sid1, np->sid2, np->sigtype);
    }
    else if (np->sigtype == 0x20)
    {
...
static int pgp_parse_pgp2_sig(unsigned char *buf, size_t l,
                              struct PgpKeyInfo *p, struct PgpSignatureNode *s)
{
...
static struct PgpKeyInfo *pgp_parse_keyblock(FILE *fp)
{
  ...
  struct PgpSignatureList lsig = STAILQ_HEAD_INITIALIZER(lsig);
  ...
  while (!err && (buf = pgp_read_packet(fp, &l)) != NULL)
  {
    ...
    switch (pt)
    {
      ...
      case PT_SIG:
      {
        if (!STAILQ_EMPTY(&lsig))
        {
          struct PgpSignatureNode *signature =
              mutt_mem_calloc(1, sizeof(struct PgpSignatureNode));
          STAILQ_INSERT_TAIL(&lsig, signature, entriens);
          //lsig = &signature->next;
	  //lsig = STAILQ_NEXT(signature, entriens);
