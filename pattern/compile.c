                      ...
                      struct Buffer *s, struct Buffer *err)
{
  ...
  bool rc = false;
  char *pexpr = s->dptr;
  if ((mutt_extract_token(buf, s, TOKEN_PATTERN | TOKEN_COMMENT) != 0) || !buf->data)
  {
                      ...
                      struct Buffer *s, struct Buffer *err, struct Mailbox *m)
{
  ...
  char *pexpr = s->dptr;
  if ((mutt_extract_token(tok_buf, s, TOKEN_PATTERN | TOKEN_COMMENT) != 0) ||
      !tok_buf->data)
  {
    mutt_buffer_printf(err, _("Error in expression: %s"), pexpr);
    goto out;
                     ...
                     struct Buffer *s, struct Buffer *err)
{
  ...
  char *pexpr = s->dptr;
  if (mutt_extract_token(tmp, s, TOKEN_COMMENT | TOKEN_PATTERN) != 0)
  {
    mutt_buffer_printf(err, _("Error in expression: %s"), pexpr);
    goto out;
  }
