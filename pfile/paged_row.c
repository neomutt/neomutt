...
void paged_row_clear(struct PagedRow *pr)
{
  ...
  paged_text_markup_clear(&pr->text);
  paged_text_markup_clear(&pr->search);
...
/**
 * paged_row_add_ansi_text - Add some text with ANSI sequences to a PagedRow
 * @param pr         PagedRow to add to
 ...
 * @retval num Number of screen columns used
 */
int paged_row_add_ansi_text(struct PagedRow *pr, const char *ansi_start,
                            const char *ansi_end, const char *text)
{
  if (!pr || !pr->paged_file || !ansi_start || !text)
    return 0;
