...
/**
 * mutt_parse_hook_global - Parse global hooks (startup, shutdown, timeout)
 * @param buf  Buffer for temporary storage
 * @param s    Buffer containing the command
 * @param data Hook type, e.g. #MUTT_STARTUP_HOOK
 * @param err  Buffer for error messages
 * @retval #CommandResult Result e.g. #MUTT_CMD_SUCCESS
 *
 * Implements Command::parse() - @ingroup command_parse
 */
static enum CommandResult mutt_parse_hook_global(struct Buffer *buf, struct Buffer *s,
                                                  intptr_t data, struct Buffer *err)
{
  ...
  parse_extract_token(cmd, s, TOKEN_SPACE);
  ...
  TAILQ_FOREACH(hook, &Hooks, entries)
  {
    ...
    if ((hook->type == data) && mutt_str_equal(hook->command, buf_string(cmd)))
    {
      rc = MUTT_CMD_SUCCESS;
      goto cleanup;
    }
...
/**
 * mutt_parse_hook_pattern - Parse pattern-based hooks (message, reply, send, send2)
 * @param buf  Buffer for temporary storage
 * @param s    Buffer containing the command
 * @param data Hook type, e.g. #MUTT_MESSAGE_HOOK
 * @param err  Buffer for error messages
 * @retval #CommandResult Result e.g. #MUTT_CMD_SUCCESS
 *
 * Implements Command::parse() - @ingroup command_parse
 */
static enum CommandResult mutt_parse_hook_pattern(struct Buffer *buf, struct Buffer *s,
                                                   intptr_t data, struct Buffer *err)
{
  struct Hook *hook = NULL;
  enum CommandResult rc = MUTT_CMD_ERROR;
...
/**
 * mutt_parse_hook_mailbox_pattern - Parse mailbox pattern hooks (fcc, fcc-save, save)
 * @param buf  Buffer for temporary storage
 * @param s    Buffer containing the command
 * @param data Hook type, e.g. #MUTT_FCC_HOOK
 * @param err  Buffer for error messages
 * @retval #CommandResult Result e.g. #MUTT_CMD_SUCCESS
 *
 * Implements Command::parse() - @ingroup command_parse
 */
static enum CommandResult mutt_parse_hook_mailbox_pattern(struct Buffer *buf, struct Buffer *s,
                                                           intptr_t data, struct Buffer *err)
{
  struct Hook *hook = NULL;
  enum CommandResult rc = MUTT_CMD_ERROR;
  ...
  struct Expando *exp = expando_parse(buf_string(cmd), IndexFormatDef, err);
...
/**
 * mutt_parse_hook_regex - Parse regex-based hooks (account)
 * @param buf  Buffer for temporary storage
 * @param s    Buffer containing the command
 * @param data Hook type, e.g. #MUTT_ACCOUNT_HOOK
 * @param err  Buffer for error messages
 * @retval #CommandResult Result e.g. #MUTT_CMD_SUCCESS
 *
 * Implements Command::parse() - @ingroup command_parse
 */
static enum CommandResult mutt_parse_hook_regex(struct Buffer *buf, struct Buffer *s,
                                                 intptr_t data, struct Buffer *err)
{
  struct Hook *hook = NULL;
  enum CommandResult rc = MUTT_CMD_ERROR;
...
/**
 * mutt_parse_hook_folder - Parse folder-hook command
 * @param buf  Buffer for temporary storage
 * @param s    Buffer containing the command
 * @param data Hook type (MUTT_FOLDER_HOOK)
 * @param err  Buffer for error messages
 * @retval #CommandResult Result e.g. #MUTT_CMD_SUCCESS
 *
 * Implements Command::parse() - @ingroup command_parse
 */
static enum CommandResult mutt_parse_hook_folder(struct Buffer *buf, struct Buffer *s,
                                                  intptr_t data, struct Buffer *err)
{
  struct Hook *hook = NULL;
  enum CommandResult rc = MUTT_CMD_ERROR;
  ...
  parse_extract_token(pattern, s, TOKEN_NO_FLAGS);
  if (mutt_str_equal(buf_string(pattern), "-noregex"))
  {
    use_regex = false;
    if (!MoreArgs(s))
    {
...
/**
 * mutt_parse_hook_crypt - Parse crypt-hook and pgp-hook commands
 * @param buf  Buffer for temporary storage
 * @param s    Buffer containing the command
 * @param data Hook type (MUTT_CRYPT_HOOK)
 * @param err  Buffer for error messages
 * @retval #CommandResult Result e.g. #MUTT_CMD_SUCCESS
 *
 * Implements Command::parse() - @ingroup command_parse
 */
static enum CommandResult mutt_parse_hook_crypt(struct Buffer *buf, struct Buffer *s,
                                                 intptr_t data, struct Buffer *err)
{
  struct Hook *hook = NULL;
  enum CommandResult rc = MUTT_CMD_ERROR;
  ...
  /* Hooks not allowing full patterns: Check syntax of regex */
  rx = MUTT_MEM_CALLOC(1, regex_t);
  int rc2 = REG_COMP(rx, buf_string(pattern), REG_ICASE);
  if (rc2 != 0)
  {
    regerror(rc2, rx, err->data, err->dsize);
    FREE(&rx);
...
/**
 * mutt_parse_hook_mbox - Parse mbox-hook command
 * @param buf  Buffer for temporary storage
 * @param s    Buffer containing the command
 * @param data Hook type (MUTT_MBOX_HOOK)
 * @param err  Buffer for error messages
 * @retval #CommandResult Result e.g. #MUTT_CMD_SUCCESS
 *
 * Implements Command::parse() - @ingroup command_parse
 */
static enum CommandResult mutt_parse_hook_mbox(struct Buffer *buf, struct Buffer *s,
                                                intptr_t data, struct Buffer *err)
{
  struct Hook *hook = NULL;
  enum CommandResult rc = MUTT_CMD_ERROR;
...
/**
 * mutt_parse_hook_compress - Parse compress hooks (append, close, open)
 * @param buf  Buffer for temporary storage
 * @param s    Buffer containing the command
 ...
 * Implements Command::parse() - @ingroup command_parse
 */
enum CommandResult mutt_parse_hook_compress(struct Buffer *buf, struct Buffer *s,
                                             intptr_t data, struct Buffer *err)
{
  struct Hook *hook = NULL;
  enum CommandResult rc = MUTT_CMD_ERROR;
...
static HookFlags mutt_get_hook_type(const char *name)
{
  ...
  ARRAY_FOREACH(cp, &NeoMutt->commands)
  {
    ...
    if (((cmd->parse == mutt_parse_hook) || (cmd->parse == mutt_parse_idxfmt_hook) ||
         (cmd->parse == mutt_parse_hook_global) || (cmd->parse == mutt_parse_hook_pattern) ||
         (cmd->parse == mutt_parse_hook_mailbox_pattern) || (cmd->parse == mutt_parse_hook_regex) ||
         (cmd->parse == mutt_parse_hook_folder) || (cmd->parse == mutt_parse_hook_crypt) ||
         (cmd->parse == mutt_parse_hook_mbox) || (cmd->parse == mutt_parse_hook_compress)) &&
...
static const struct Command HookCommands[] = {
  ...
  { "crypt-hook",        mutt_parse_hook_crypt,         MUTT_CRYPT_HOOK },
  { "fcc-hook",          mutt_parse_hook_mailbox_pattern, MUTT_FCC_HOOK },
  { "fcc-save-hook",     mutt_parse_hook_mailbox_pattern, MUTT_FCC_HOOK | MUTT_SAVE_HOOK },
  { "folder-hook",       mutt_parse_hook_folder,        MUTT_FOLDER_HOOK },
  { "iconv-hook",        mutt_parse_charset_iconv_hook, MUTT_ICONV_HOOK },
  { "index-format-hook", mutt_parse_idxfmt_hook,        MUTT_IDXFMTHOOK },
  { "mbox-hook",         mutt_parse_hook_mbox,          MUTT_MBOX_HOOK },
  { "message-hook",      mutt_parse_hook_pattern,       MUTT_MESSAGE_HOOK },
  { "pgp-hook",          mutt_parse_hook_crypt,         MUTT_CRYPT_HOOK },
  { "reply-hook",        mutt_parse_hook_pattern,       MUTT_REPLY_HOOK },
  { "save-hook",         mutt_parse_hook_mailbox_pattern, MUTT_SAVE_HOOK },
  { "send-hook",         mutt_parse_hook_pattern,       MUTT_SEND_HOOK },
  { "send2-hook",        mutt_parse_hook_pattern,       MUTT_SEND2_HOOK },
  { "shutdown-hook",     mutt_parse_hook_global,        MUTT_SHUTDOWN_HOOK | MUTT_GLOBAL_HOOK },
  { "startup-hook",      mutt_parse_hook_global,        MUTT_STARTUP_HOOK | MUTT_GLOBAL_HOOK },
  { "timeout-hook",      mutt_parse_hook_global,        MUTT_TIMEOUT_HOOK | MUTT_GLOBAL_HOOK },
